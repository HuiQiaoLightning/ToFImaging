...
parfor xy = 1:size(measurements,2)
    ...
    %}
    tmp = rec_pixel_2_coef(M, weight, waveletname, N, num_noise, tmp, sp);    
...
if isfield(mM,'simu')
    ...
    [y,x] = deal(70,42);             % 分析具体的像素点：(60,70);(30,40);(149,88);(83,136);(150,91);(131,67);(86,31);(95,148);(57,92);(45,53); (75,164)
    rp = squeeze(I_rec(x,y,:));
...
function rec_ap = rec_pixel_2_coef(M, weight, waveletname, N, num_noise, single_measurement, sp)
nn = 1;
while nn <= num_noise     
...
for nn = 1:num_noise
    for level = 1:size( ap,2 )  
        ...
        coef = max(abs(tmp)) / max(abs(tmp1));    % 计算线性系数，满足低频特性，反正高频计算也不准确
        tmp1 = coef *tmp1;
%         figure;plot([tmp tmp1]);
        similarity( nn,level ) = norm(tmp - tmp1);
        %{
        figure;plot([ap{nn,level} sp]);
        figure;plot([coef * ap{nn,level} sp]);
        %}
        ap{nn,level} = coef * ap{nn,level};
