...
trans_img = TransientwithLow_90(wHc1,freq1,freqstep,tau0,tau_step,tau_len);
...
% trans_iron = trans_iron/max(trans_iron(:));
...
%     'tau_step','tau_len','freq','freq1','freq2','freqstep','freq_len');
...
%% show video
% trans_iron = IronTransient3(trans_img,1,10);
trans_iron = PulsizeTransient(trans_img,freq2,tau_step,0.05,0.1);
% norm_img = NormalizeImage(trans_iron,2);
norm_img = CompressHDR(trans_iron);

i0 = floor(0/tau_step);
i_len = floor(16/tau_step);
for i=1:i_len
    figure(10)
